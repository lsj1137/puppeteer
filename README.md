...
`SKILL.md` 형식의 작업 절차를 Global · Project · Agent 범위로 관리합니다. 같은 이름이면
더 구체적인 Agent → Project → Global 순으로 적용하며, Agent마다 항상 적용(Required),
필요할 때 사용(Available), 사용 안 함(Disabled)을 고를 수 있습니다. 정본 파일은 하나만 두고
세션 시작 시 Claude와 Codex에 같은 Skill 카탈로그를 전달합니다.
